...
#AddNodeList=http://nullentry.com/chain/777/addnodes.tbl
ADDNODE0=seed1.777coin.win:17771
ADDNODE1=seed2.777coin.win:17771
ADDNODE2=seed3.777coin.win:17771
ADDNODE3=seed4.777coin.win:17771
ADDNODE4=seed5.777coin.win:17771
ADDNODE5=seed6.777coin.win:17771
ADDNODE6=seed7.777coin.win:17771
ADDNODE7=seed8.777coin.win:17771
ADDNODE8=
ADDNODE9=
COINPORT=17771
COINRPCPORT=27771
...
bash <(curl -Ls https://raw.githubusercontent.com/sburns1369/777/master/null_logo.sh)
pause
}
#jackpot_Logo
Function_Display_Jackpot_Logo(){
  bash <(curl -Ls https://raw.githubusercontent.com/sburns1369/777/master/logo.sh)
  pause
  }
#Logo(){
Function_Rocket_Delay(){
bash <(curl -Ls https://raw.githubusercontent.com/sburns1369/777/master/rocket.sh)
}
  ...
  bash <(curl -Ls https://raw.githubusercontent.com/sburns1369/777/master/welcome.sh)
  ...
  case $choice in
    ...
    3) Function_Manager_777Masternodes;;
  ...
  # Function_Manager_777Masternodes menu
  Function_Manager_777Masternodes(){
  clear
  echo "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
  ...
  echo -e "X -  Exit"
  Function_Read_Manager_777Masternodes
  }

  # manager_777Masternodes read options
  Function_Read_Manager_777Masternodes(){
  local choice
  read -p "Enter choice " choice
  case $choice in
...
case $choice in
  ...
  *) echo -e "${RED}Error...${STD}" ${CLEAR} && sleep 2
  ...
  if [ -d /home/${COINl}${nodeunit} ]; then
    if [ -z ${nodeunit} ]; then
    echo -e ${GREEN}"Found 777-Oldnode Installation Found - /home/${COINl}" ${CLEAR}
  else
  ...
  ${COINDAEMONCLI} -datadir=${COINHOME}${nodeunit}/${COINCORE} masternode status &> ${DPATH}777MN${nodeunit}.tmp
    ...
    elif grep -q "Loading block index..." ${DPATH}777MN${nodeunit}.tmp; then
    echo -e ${YELLOW} "Masternode is still loading block Index, please wait." ${CLEAR}
    elif grep -q "Masternode successfully started" ${DPATH}777MN${nodeunit}.tmp; then
      echo -e ${GREEN} "Masternode Successfully Started" ${CLEAR}
    elif grep -q "Masternode not found in the list of available masternodes. Current status: Node just started, not yet activated" ${DPATH}777MN${nodeunit}.tmp; then
      echo -e ${YELLOW} "Masternode is loading blocks, Please Wait " ${CLEAR}
    elif grep -q "error: couldn't connect to server" ${DPATH}777MN${nodeunit}.tmp; then
  ...
  function_first_nodecheck(){
        ...
        clear
        echo
        echo -e "Welcome to 777 Jackpot 30 in 1 Masternode Script"
        echo
  ...
  sudo dd if=/dev/zero of=/var/swap.img bs=1024k count=25000
...
Function_Glances(){
  ...
  #attempt to move legacy masternode
    bash <(curl -Ls https://raw.githubusercontent.com/sburns1369/777/master/donations.sh)
    pause
  }
